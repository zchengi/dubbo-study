...
public class DefaultAlipayServiceImpl implements AlipayServiceAPI {
    ...
    public String trade_precreate(String orderId) {
        ...
        String body = "购买花费: " + orderVO.getOrderPrice();

        // 商户操作员编号，添加此参数可以为商户操作员做销售统计
        String operatorId = "cheng";

        // (必填) 商户门店编号，通过门店号和商家后台可以配置精准到门店的折扣信息，详询支付宝技术支持
        String storeId = "cheng";
...
//        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                filePath = String.format("D:/Appserv/ftp/tmp/qr-%s.png",
                ...
                // 上传到 ftp
                boolean success = ftpUtil.uploadFile(fileName, qrCodeImge);
                if (!success) {
                    filePath = "";
                    log.error("二维码上传失败");
                }
